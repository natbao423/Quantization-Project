...
print("\nrounding behavior")
x = torch.tensor([1.0, 1.001, 1.01, 0.1, 1e-8, 1e30], device=DEVICE)
print("fp32:", x)
...
for K in (16, 64, 256, 1024, 4096): #inner dimension, # of terms summed per output element
    A, B = torch.randn(256, K, device=DEVICE), torch.randn(K, 256, device=DEVICE) 
    #builds two random tensors, FP32 by default
...
out = ROOT / "results" / "data" / "precision_basics.csv"
